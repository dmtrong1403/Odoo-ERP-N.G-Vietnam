...
class custom_purchase_order(models.Model):
    ...
    contract_status = fields.Selection(cv.CONTRACT_STATUS, string="Tình trạng hợp đồng")
    tag_ids = fields.Many2many(comodel_name="purchase.request.tag", string="Tag name")
    state = fields.Selection(cv.PO_STATUS, string="Status", readonly=True, index=True, copy=False, default="draft",
                             track_visibility="onchange")
    purchase_contract_line_ids = fields.One2many(comodel_name="purchase.contract.line",
                                                 inverse_name="purchase_order_id")
    purchase_request_id = fields.Many2one(comodel_name="purchase.request", string="Yêu cầu mua hàng",
...
class custom_purchase_orderline(models.Model):
    ...
    rating_by_po = fields.Selection(cv.RATING_STATUS, string="Đánh giá")
