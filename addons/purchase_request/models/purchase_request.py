...
class purchase_request(models.Model):
    ...
    user_id = fields.Many2one(comodel_name="res.users", string="Người đề xuất",
                              default=lambda self: self.get_current_user())
    department_id = fields.Many2one(comodel_name="hr.department", string="Phòng ban",
                                    default=lambda self: self.get_current_department())
    company_id = fields.Many2one(comodel_name="res.company", string="Trang trại / Công ty",
                                 default=lambda self: self.get_current_company())
    ...
    propose = fields.Char(string="Mục đích")
    note = fields.Text()
